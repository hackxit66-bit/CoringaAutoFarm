...
-- =============================================
-- FUNCOES DAS UTILIDADES CORRIGIDAS
-- =============================================
...
-- ESPIAR JOGADOR CORRIGIDO (SEM BUG)
function espiarJogador(alvo)
    if espiandoAtivo then return end
    espiandoAtivo = true
    
    spawn(function()
        local tempo = 0
        while getgenv().CoringaFarm.Espiando == alvo and tempo < 30 do
            local root = getRoot()
            ...
            if root and alvoChar and alvoChar:FindFirstChild("HumanoidRootPart") then
                ...
                root.CFrame = CFrame.new(alvoPos.x, alvoPos.y + 5, alvoPos.z + 5)
            end
            wait(0.2)
            tempo = tempo + 0.2
        end
        getgenv().CoringaFarm.Espiando = nil
        espiandoAtivo = false
    end)
end
...
-- TELEPORTAR PARA JOGADOR CORRIGIDO (SEM BUG)
...
-- =============================================
-- FUNCAO DE FARM CORRIGIDA (OURO POR FASE)
-- =============================================
...
function autoFarm(currentRun)
    ...
    -- PASSA POR TODAS AS 10 FASES E GANHA OURO A CADA FASE
    for i = 1, 10 do
        ...
        if Stage then
            ...
            if DarknessPart then
                ...
                faseAtual = i
                ouroTotal = ouroTotal + (i * 10) -- +10 na fase 1, +20 na fase 2, etc.
                atualizarUI()
            end
        end
    end

    -- VAI PARA O BAÚ E GANHA MAIS OURO
        ...
        if TheEnd then
            ...
            if GoldenChest then
                ...
                if Trigger then
                    ...
                    if luzMudou then
                        ouroTotal = ouroTotal + 100 -- Bonus do bau
...
-- =============================================
-- PAINEL DIREITO (JOGADORES / TIMES / PLAYER / CONFIG / OPCOES)
-- =============================================
...
btnEspiar.MouseButton1Click:Connect(function()
    if jogadorSelecionado then
        getgenv().CoringaFarm.Espiando = jogadorSelecionado
...
function atualizarIdioma()
    ...
    rightTitle.Text = mostrandoJogadores and textos[idiomaAtual].jogadores or 
                     ...
                     (mostrandoOpcoesJogador and (jogadorSelecionado and jogadorSelecionado.Name or "Opcoes") or 
