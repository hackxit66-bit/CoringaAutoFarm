...
function antiAfkLoop()
    spawn(function()
        while getgenv().CoringaFarm.AntiAfk do
            -- 1. Movimento suave da câmera
            local camera = Workspace.CurrentCamera
            if camera then
                local rotacao = camera.CFrame - camera.CFrame.p
                camera.CFrame = CFrame.new(camera.CFrame.p) * CFrame.Angles(0, math.rad(0.1), 0) * rotacao
                wait(0.5)
                camera.CFrame = CFrame.new(camera.CFrame.p) * CFrame.Angles(0, math.rad(-0.1), 0) * rotacao
            end
            ...
            -- 3. Movimento mínimo do personagem
            local root = getRoot()
            if root then
            ...
            wait(25) -- Executa a cada 25 segundos
        end
    end)
end
...
-- =============================================
-- FUNÇÕES DE VOO (APENAS SUBIR E DESCER)
-- =============================================
...
-- =============================================
-- FUNÇÕES DE TELEPORTE E JOGADORES
-- =============================================
