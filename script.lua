...
local textos = {
    portugues = {
        ...
        efeitosNome = " Efeitos",
        noclipNome = " NoClip",
        autoBuilderNome = " Auto Builder",
        vidaInfinitaNome = " Vida Infinita",
        farmInteligenteNome = " Farm Inteligente"
    },
    english = {
        titulo = " CORINGA ",
        iniciar = " START",
        ...
        farmInteligenteNome = " Smart Farm"
    },
    espanol = {
        titulo = " CORINGA ",
        iniciar = " INICIAR",
        ...
        efeitosNome = " Efectos",
        noclipNome = " NoClip",
        autoBuilderNome = " Auto Constructor",
        vidaInfinitaNome = " Vida Infinita",
        farmInteligenteNome = " Farm Inteligente"
    }
}
...
-- AUTO BUILDER
function autoBuilderLoop()
    spawn(function()
        while getgenv().CoringaFarm.AutoBuilder do
            local root = getRoot()
            if root then
                local block = Instance.new("Part")
                block.Size = Vector3.new(4, 1, 4)
                block.Position = root.Position + Vector3.new(0, -3, 5)
...
-- FARM INTELIGENTE
function farmInteligenteLoop()
    spawn(function()
        while getgenv().CoringaFarm.FarmInteligente do
            local root = getRoot()
            if root then
                local bestItem = nil
                ...
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("Part") and obj.Position then
                        if obj.Name:find("Chest") or obj.Name:find("Coin") or obj.Name:find("Gold") then
                ...
                if bestItem then
                    root.CFrame = bestItem.CFrame * CFrame.new(0, 2, 0)
                end
            end
            wait(0.5)
        end
...
-- LISTA DE PLAYER (COM TODAS AS FUNÇÕES NOVAS)
...
local function criarBotoesPlayer()
    ...
    local functions = {
        ...
        {nome = textos[idiomaAtual].autoBuilderNome, var = "AutoBuilder"},
        {nome = textos[idiomaAtual].vidaInfinitaNome, var = "VidaInfinita"},
        {nome = textos[idiomaAtual].farmInteligenteNome, var = "FarmInteligente"}
    }
    ...
    for i, func in pairs(functions) do
        ...
        btnToggle.MouseButton1Click:Connect(function()
            ...
            elseif func.var == "Voo" then vooLoop()
            ...
            elseif func.var == "AutoBuilder" then autoBuilderLoop()
            elseif func.var == "VidaInfinita" then vidaInfinitaLoop()
            elseif func.var == "FarmInteligente" then farmInteligenteLoop()
            end
        end)
