...
-- =============================================
-- FUNÇÕES DE UTILIDADES CORRIGIDAS
-- =============================================

-- VIDA INFINITA CORRIGIDA (também contra afogamento)
function vidaInfinitaLoop()
    spawn(function()
        ...
        -- Restaurar estados ao desativar
        local character = player.Character
        if character then
            local humanoid = character:FindFirstChild("Humanoid")
            if humanoid then
...
-- ANDAR NA ÁGUA CORRIGIDO (com bloco e respiração)
    ...
    spawn(function()
        while getgenv().CoringaFarm.AndarAgua do
            ...
            if root and humanoid then
                -- Criar bloco invisível se não existir
                if not blocoAgua then
                    ...
                    blocoAgua.CanTouch = true
                    blocoAgua.Parent = Workspace
                end
                
                -- Posicionar bloco abaixo do jogador
                blocoAgua.CFrame = root.CFrame - Vector3.new(0, 3.5, 0)
                
                -- Verificar se está na água
                local material = Workspace:GetMaterialAtPosition(root.Position + Vector3.new(0, -2, 0))
                if material == Enum.Material.Water then
                    -- Manter acima da água
                    root.CFrame = root.CFrame + Vector3.new(0, 4, 0)
                    
                    -- Impedir estado de natação
                    humanoid:SetStateEnabled(Enum.HumanoidStateType.Swimming, false)
...
-- ESCUDO CORRIGIDO (agora some ao desativar)
function escudoLoop()
    spawn(function()
...
function criarESP(jogador)
    ...
    billboard.Name = "ESP_Nome_" .. jogador.Name
    billboard.Size = UDim2.new(0, 200, 0, 50)
    billboard.StudsOffset = Vector3.new(0, 3, 0)
    billboard.AlwaysOnTop = true
    billboard.Parent = jogador.Character:FindFirstChild("Head") or jogador.Character:FindFirstChild("HumanoidRootPart")
    
    local texto = Instance.new("TextLabel")
    texto.Size = UDim2.new(1, 0, 1, 0)
    texto.BackgroundTransparency = 1
    texto.Text = jogador.Name .. " | " .. math.floor((jogador.Character.HumanoidRootPart.Position - getRoot().Position).Magnitude) .. "m"
...
function antiAfkLoop()
    spawn(function()
        while getgenv().CoringaFarm.AntiAfk do
            ...
            if camera then
                local rotacao = camera.CFrame - camera.CFrame.p
                camera.CFrame = CFrame.new(camera.CFrame.p) * CFrame.Angles(0, math.rad(0.2), 0) * rotacao
                wait(0.3)
                camera.CFrame = CFrame.new(camera.CFrame.p) * CFrame.Angles(0, math.rad(-0.2), 0) * rotacao
            end
