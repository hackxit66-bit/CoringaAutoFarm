...
local tamanhoOriginal = UDim2.new(0, 400, 0, 280)
...
function teleportarParaCor(corInfo)
    ...
    -- Primeiro tenta encontrar jogadores da mesma cor
    for _, otherPlayer in pairs(Players:GetPlayers()) do
        if otherPlayer ~= player then
            local otherChar = otherPlayer.Character
            if otherChar then
    ...
    -- Se não encontrar, tenta mudar a própria cor
...
botoesFrame.Size = UDim2.new(1, 0, 0, 150)
...
btnEscudo.Position = UDim2.new(0, 0, 0, 105)
...
-- =============================================
-- PAINEL DIREITO (JOGADORES / TIMES)
-- =============================================
...
-- LISTA DE TIMES (AGORA COM 7 CORES)
...
btnTimes.MouseButton1Click:Connect(function()
    ...
    jogadoresLista.Visible = mostrandoJogadores
    timesLista.Visible = not mostrandoJogadores
    rightTitle.Text = mostrandoJogadores and " JOGADORES" or " TIMES"
    btnTimes.Text = mostrandoJogadores and " TIMES" or " FECHAR TIMES"
end)
