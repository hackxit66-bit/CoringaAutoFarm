...
local tamanhoOriginal = UDim2.new(0, 400, 0, 320)
...
function speedBoostLoop()
    spawn(function()
        while getgenv().CoringaFarm.SpeedBoost do
...
function highJumpLoop()
    spawn(function()
        while getgenv().CoringaFarm.HighJump do
            local humanoid = getHumanoid()
            if humanoid then
        ...
        if otherPlayer ~= player then
            ...
            if otherChar then
                for _, part in pairs(otherChar:GetChildren()) do
                    if part:IsA("Part") and part.BrickColor == BrickColor.new(corInfo.Cor) then
        ...
        if character then
            for _, part in pairs(character:GetChildren()) do
                if part:IsA("Part") then
                    part.BrickColor = BrickColor.new(corInfo.Cor)
                end
            end
        end
    end
...
-- =============================================
-- FUNÇÃO DE FARM CORRIGIDA (INDO ATÉ OS BAÚS)
-- =============================================
...
function autoFarm(currentRun)
    ...
    for i = 1, 10 do
        ...
        if Stage then
            ...
            if DarknessPart then
                -- Teleporta para a parte escura da fase
    ...
    if farmAtivo and not farmCancelado then
        ...
        if TheEnd then
            ...
            if GoldenChest then
                -- Teleporta para o baú dourado
                Character.HumanoidRootPart.CFrame = GoldenChest.CFrame * CFrame.new(0, 5, 0)
                wait(1)
                
                -- Tenta pegar o trigger do baú
                ...
                -- Adiciona ouro por completar a fase
...
botoesFrame.Size = UDim2.new(1, 0, 0, 190)
...
btnPlayer.BackgroundColor3 = Color3.fromRGB(0, 150, 150)
...
btnEscudo.Size = UDim2.new(1, 0, 0, 30)
btnEscudo.Position = UDim2.new(0, 0, 0, 140)
...
-- LISTA DE PLAYER
local playerLista = Instance.new("Frame")
playerLista.Size = UDim2.new(1, -10, 0, 180)
...
local playerFunctions = {
    {nome = " Speed Boost", var = "SpeedBoost", cor = Color3.fromRGB(255, 100, 0)},
    {nome = " Invisível", var = "Invisivel", cor = Color3.fromRGB(100, 100, 100)},
    {nome = " High Jump", var = "HighJump", cor = Color3.fromRGB(0, 150, 255)},
    {nome = " Escudo", var = "Escudo", cor = Color3.fromRGB(0, 0, 150)}
}

local yPos = 5
for i, func in pairs(playerFunctions) do
    ...
    btnToggle.MouseButton1Click:Connect(function()
        ...
        if func.var == "SpeedBoost" then speedBoostLoop()
        elseif func.var == "Invisivel" then invisivelLoop()
        elseif func.var == "HighJump" then highJumpLoop()
...
btnTimes.MouseButton1Click:Connect(function()
    ...
    playerLista.Visible = false
    rightTitle.Text = " TIMES"
    btnTimes.Text = " FECHAR TIMES"
    btnPlayer.Text = " PLAYER"
end)
...
btnPlayer.MouseButton1Click:Connect(function()
    ...
    rightTitle.Text = " PLAYER"
    btnPlayer.Text = " FECHAR PLAYER"
    btnTimes.Text = " TIMES"
end)
...
spawn(function()
    while true do
        wait(3)
        if jogadoresLista.Visible then
