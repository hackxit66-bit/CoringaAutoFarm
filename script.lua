...
-- FUNÇÃO CORRIGIDA PARA TELEPORTAR POR COR
function teleportarParaCor(corInfo)
    local root = getRoot()
    if not root then return end
    
    -- Primeiro tenta encontrar jogador com essa cor
    for _, otherPlayer in pairs(Players:GetPlayers()) do
        if otherPlayer ~= player then
            local otherChar = otherPlayer.Character
            if otherChar and otherChar:FindFirstChild("HumanoidRootPart") then
                -- Verifica se o jogador tem a cor
                for _, part in pairs(otherChar:GetChildren()) do
                    if part:IsA("Part") and part.BrickColor == corInfo.TeamColor then
                        ...
                        print(" Teleportado para jogador da cor " .. corInfo.Nome)
                        return
                    end
                end
            end
    ...
    -- Se não achar, muda a própria cor
    local character = player.Character
    if character then
        for _, part in pairs(character:GetChildren()) do
        ...
        print(" Cor alterada para " .. corInfo.Nome)
    end
end

-- =============================================
-- FUNÇÃO DE FARM CORRIGIDA (COMO NAS PRIMEIRAS MENSAGENS)
-- =============================================
...
function autoFarm(currentRun)
    ...
    -- Verifica se as estruturas existem
    ...
    if farmAtivo and not farmCancelado then
        ...
        if TheEnd then
            ...
            if GoldenChest then
                ...
                print(" +100 ouro! Total: " .. ouroTotal)
                atualizarUI()
            end
        end
    end
...
-- LISTA DE PLAYER (COM FUNÇÕES EM PORTUGUÊS)
...
local playerFunctions = {
    {nome = " Velocidade", var = "Velocidade", cor = Color3.fromRGB(200, 200, 200)},
    {nome = " Invisível", var = "Invisivel", cor = Color3.fromRGB(200, 200, 200)},
    {nome = " Super Pulo", var = "SuperPulo", cor = Color3.fromRGB(200, 200, 200)},
    {nome = " Escudo", var = "Escudo", cor = Color3.fromRGB(200, 200, 200)}
}
