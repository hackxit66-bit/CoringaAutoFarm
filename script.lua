...
-- =============================================
-- =============================================
-- FUNCOES DOS EFEITOS E UTILIDADES (COM VOO CORRIGIDO)
-- =============================================
...
-- =============================================
-- FUNÇÃO DE VOO CORRIGIDA (MANTÉM NO AR)
-- =============================================
...
function criarBotoesVoo()
    ...
    -- Criar BodyVelocity para manter no ar
    bodyVelocityVoo = Instance.new("BodyVelocity")
    bodyVelocityVoo.MaxForce = Vector3.new(0, 4000, 0) -- Só força na vertical
    ...
    -- =============================================
    -- LADO ESQUERDO - JOYSTICK (4 BOTÕES)
    -- =============================================
    
    -- Botão CIMA (move para frente no eixo Z)
    ...
    -- Botão BAIXO (move para trás no eixo Z)
    ...
    -- Botão ESQUERDA (move para esquerda no eixo X)
    ...
    -- Botão DIREITA (move para direita no eixo X)
    ...
    -- =============================================
    -- LADO DIREITO - CONTROLES VERTICAIS (2 BOTÕES)
    -- =============================================
    
    -- Botão SUBIR (sobe no eixo Y e mantém)
    ...
    btnSubir.MouseButton1Click:Connect(function()
        local root = getRoot()
        if root and vooAtivo then
            -- Sobe e mantém a posição
            root.CFrame = root.CFrame + Vector3.new(0, 10, 0)
            -- Atualiza a força para manter a nova altura
            if bodyVelocityVoo then
                bodyVelocityVoo.Velocity = Vector3.new(0, 0, 0)
            end
        end
    ...
    -- Botão DESCER (desce no eixo Y e mantém)
...
-- FUNCOES DE UTILIDADES EXISTENTES
function escudoLoop()
    spawn(function()
        ...
        if func.tipo == "toggle" then
            ...
            btnToggle.MouseButton1Click:Connect(function()
                ...
                elseif func.var == "Voo" then 
                    if getgenv().CoringaFarm.Voo then
                        criarBotoesVoo()
                        vooLoop()
                    else
                        destruirBotoesVoo()
