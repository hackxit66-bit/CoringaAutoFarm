...
-- =============================================
-- FUNÇÕES DE UTILIDADES (TODAS FUNCIONANDO)
-- =============================================
...
-- VIDA INFINITA FUNCIONAL (água e quedas)
    ...
    spawn(function()
        while getgenv().CoringaFarm.VidaInfinita do
            ...
            if character then
                ...
                if root then
                    local material = Workspace:GetMaterialAtPosition(root.Position)
    ...
    spawn(function()
        ...
        while getgenv().CoringaFarm.AndarAgua do
            ...
            if root and humanoid then
                if not bloco then
                    bloco = Instance.new("Part")
                    bloco.Anchored = true
                    bloco.Size = Vector3.new(8, 1, 8)
                    bloco.Transparency = 1
                    bloco.CanCollide = true
                    bloco.CanTouch = false
                    bloco.Parent = Workspace
                end
                
                bloco.CFrame = root.CFrame - Vector3.new(0, 3, 0)
...
function criarESP(jogador)
    ...
    local distancia = (jogador.Character.HumanoidRootPart.Position - getRoot().Position).Magnitude
    texto.Text = jogador.Name .. " | " .. math.floor(distancia) .. "m"
