...
-- =============================================
-- ESCUDO SUPER SIMPLES (FUNCIONA COM CERTEZA)
function escudoLoop()
    spawn(function()
        -- Enquanto o botão estiver ativado
        while getgenv().CoringaFarm.Escudo do
            local character = player.Character
            if character then
                -- Verifica se já tem escudo
        ...
        -- Quando desativar, remove o escudo
        local character = player.Character
        if character then
...
-- EFEITO DE BOLINHAS (O PRIMEIRO QUE FUNCIONAVA)
    ...
    spawn(function()
        while getgenv().CoringaFarm.Efeitos do
            ...
            if root then
                ...
                -- Criar partículas douradas (bolinhas)
