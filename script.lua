...
local textos = {
    portugues = {
        ...
        avancado = " AVANÇADO",
        fecharAvancado = " FECHAR AVANÇADO",
        config = " CONFIG",
        fecharConfig = " FECHAR CONFIG",
        jogadores = " JOGADORES",
        timesTitulo = " TIMES",
        playerTitulo = " PLAYER",
        avancadoTitulo = " AVANÇADO",
        ...
        autoDestruirNome = " Auto Destruir",
        prioridadeNome = " Prioridade",
        modoAmigoNome = " Modo Amigo",
        localizarNome = " Localizar",
        autoColetarNome = " Auto Coletar",
        miniMapaNome = " Mini Mapa",
        antiQuedaNome = " Anti Queda",
        superForcaNome = " Super Força"
    },
    english = {
        titulo = " CORINGA ",
        iniciar = " START",
        ...
        avancado = " ADVANCED",
        fecharAvancado = " CLOSE ADVANCED",
        config = " SETTINGS",
        fecharConfig = " CLOSE SETTINGS",
        jogadores = " PLAYERS",
        timesTitulo = " TEAMS",
        playerTitulo = " PLAYER",
        avancadoTitulo = " ADVANCED",
        ...
        autoDestruirNome = " Auto Destroy",
        prioridadeNome = " Priority",
        modoAmigoNome = " Friend Mode",
        localizarNome = " Locate",
        autoColetarNome = " Auto Collect",
        miniMapaNome = " Mini Map",
        antiQuedaNome = " Anti Fall",
        superForcaNome = " Super Strength"
    },
    espanol = {
        titulo = " CORINGA ",
        iniciar = " INICIAR",
        ...
        avancado = " AVANZADO",
        fecharAvancado = " CERRAR AVANZADO",
        config = " CONFIG",
        fecharConfig = " CERRAR CONFIG",
        jogadores = " JUGADORES",
        timesTitulo = " EQUIPOS",
        playerTitulo = " JUGADOR",
        avancadoTitulo = " AVANZADO",
        ...
        vidaInfinitaNome = " Vida Infinita",
        autoDestruirNome = " Auto Destruir",
        prioridadeNome = " Prioridad",
        modoAmigoNome = " Modo Amigo",
        localizarNome = " Localizar",
        autoColetarNome = " Auto Colectar",
        miniMapaNome = " Mini Mapa",
        antiQuedaNome = " Anti Caída",
        superForcaNome = " Super Fuerza"
    }
}
...
-- =============================================
-- FUNÇÕES DAS UTILIDADES AVANÇADAS
-- =============================================

-- AUTO DESTRUIR
function autoDestruirLoop()
    spawn(function()
        while getgenv().CoringaFarm.AutoDestruir do
            local root = getRoot()
            if root then
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("Part") and obj.Position then
                        local dist = (obj.Position - root.Position).Magnitude
                        if dist < 20 and obj.Name ~= "Base" and not obj.Anchored then
                            obj:Destroy()
                        end
                    end
                end
...
-- PRIORIDADE (vai para itens importantes)
function prioridadeLoop()
    spawn(function()
        while getgenv().CoringaFarm.Prioridade do
            local root = getRoot()
            if root then
                local prioridadeItens = {"Chest", "Coin", "Gold", "Treasure"}
                local melhorItem = nil
                local menorDist = math.huge
                
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("Part") and obj.Position then
                        for _, nome in pairs(prioridadeItens) do
                ...
                if melhorItem then
                    root.CFrame = melhorItem.CFrame * CFrame.new(0, 2, 0)
                end
            end
            wait(0.5)
        end
                    ...
                    if otherPlayer ~= player then
                        ...
                        if otherChar and otherChar:FindFirstChild("HumanoidRootPart") then
                            ...
                            if dist < 10 and not amigos[otherPlayer.Name] then
                                table.insert(amigos, otherPlayer.Name)
                            end
                        end
                    end
                end
...
-- LOCALIZAR (mostra amigos no mapa)
function localizarLoop()
    spawn(function()
        while getgenv().CoringaFarm.Localizar do
            print(" Amigos próximos:")
            for _, nome in pairs(amigos) do
                local amigo = Players:FindFirstChild(nome)
                if amigo and amigo.Character and amigo.Character:FindFirstChild("HumanoidRootPart") then
                    local pos = amigo.Character.HumanoidRootPart.Position
                    print("   " .. nome .. " - X: " .. math.floor(pos.X) .. " Y: " .. math.floor(pos.Y) .. " Z: " .. math.floor(pos.Z))
                end
            end
            wait(5)
        end
...
-- AUTO COLETAR
function autoColetarLoop()
    spawn(function()
        while getgenv().CoringaFarm.AutoColetar do
            local root = getRoot()
            if root then
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("Part") and obj.Position then
                        if obj.Name:find("Coin") or obj.Name:find("Chest") or obj.Name:find("Gold") then
                            local dist = (obj.Position - root.Position).Magnitude
                            if dist < 30 then
...
-- MINI MAPA
function miniMapaLoop()
    spawn(function()
        while getgenv().CoringaFarm.MiniMapa do
            if not miniMapa then
                miniMapa = Instance.new("ScreenGui")
                miniMapa.Name = "MiniMapa"
                miniMapa.Parent = player.PlayerGui
                
                local mapaFrame = Instance.new("Frame")
                mapaFrame.Size = UDim2.new(0, 200, 0, 200)
                mapaFrame.Position = UDim2.new(0.8, 0, 0.05, 0)
                mapaFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
                mapaFrame.BackgroundTransparency = 0.3
                mapaFrame.Parent = miniMapa
                Instance.new("UICorner", mapaFrame).CornerRadius = UDim.new(0, 10)
                
                miniMapaCamera = Instance.new("Camera")
                miniMapaCamera.CameraType = Enum.CameraType.Scriptable
                miniMapaCamera.Parent = mapaFrame
            end
            
            if miniMapaCamera then
                local root = getRoot()
                if root then
                    miniMapaCamera.CFrame = CFrame.new(root.Position + Vector3.new(0, 50, 0), root.Position)
                end
            end
            wait()
        end
...
-- SUPER FORÇA
function superForcaLoop()
    spawn(function()
        while getgenv().CoringaFarm.SuperForca do
            local root = getRoot()
            if root then
                for _, obj in pairs(Workspace:GetDescendants()) do
                    if obj:IsA("Part") and obj.Position then
                        local dist = (obj.Position - root.Position).Magnitude
                        if dist < 15 and obj:FindFirstChild("TouchInterest") then
...
-- BOTÕES EM CINZA CLARO (AGORA COM AVANÇADO)
local botoesFrame = Instance.new("Frame")
botoesFrame.Size = UDim2.new(1, 0, 0, 210)
...
local btnAvancado = Instance.new("TextButton")
btnAvancado.Size = UDim2.new(1, 0, 0, 30)
btnAvancado.Position = UDim2.new(0, 0, 0, 140)
btnAvancado.BackgroundColor3 = Color3.fromRGB(200, 200, 200)
btnAvancado.Text = textos[idiomaAtual].avancado
btnAvancado.TextColor3 = Color3.fromRGB(0, 0, 0)
btnAvancado.TextSize = 10
btnAvancado.Font = Enum.Font.GothamBold
btnAvancado.Parent = botoesFrame
Instance.new("UICorner", btnAvancado).CornerRadius = UDim.new(0, 4)

local btnConfig = Instance.new("TextButton")
btnConfig.Size = UDim2.new(1, 0, 0, 30)
btnConfig.Position = UDim2.new(0, 0, 0, 175)
...
-- =============================================
-- PAINEL DIREITO (JOGADORES / TIMES / PLAYER / AVANÇADO / CONFIG)
-- =============================================
...
-- LISTA DE AVANÇADO
local avancadoLista = Instance.new("ScrollingFrame")
avancadoLista.Size = UDim2.new(1, -10, 1, -30)
avancadoLista.Position = UDim2.new(0, 5, 0, 25)
avancadoLista.BackgroundTransparency = 1
avancadoLista.ScrollBarThickness = 4
avancadoLista.CanvasSize = UDim2.new(0, 0, 0, 0)
avancadoLista.Parent = rightPanel
avancadoLista.Visible = false

local function criarBotoesAvancado()
    for _, child in pairs(avancadoLista:GetChildren()) do
        child:Destroy()
    end
    
    local functions = {
        {nome = textos[idiomaAtual].autoDestruirNome, var = "AutoDestruir"},
        {nome = textos[idiomaAtual].prioridadeNome, var = "Prioridade"},
        {nome = textos[idiomaAtual].modoAmigoNome, var = "ModoAmigo"},
        {nome = textos[idiomaAtual].localizarNome, var = "Localizar"},
        {nome = textos[idiomaAtual].autoColetarNome, var = "AutoColetar"},
        {nome = textos[idiomaAtual].miniMapaNome, var = "MiniMapa"},
        {nome = textos[idiomaAtual].antiQuedaNome, var = "AntiQueda"},
        {nome = textos[idiomaAtual].superForcaNome, var = "SuperForca"}
    }
    ...
    for i, func in pairs(functions) do
        ...
        btnToggle.MouseButton1Click:Connect(function()
            ...
            if func.var == "AutoDestruir" then autoDestruirLoop()
            elseif func.var == "Prioridade" then prioridadeLoop()
            elseif func.var == "ModoAmigo" then modoAmigoLoop()
            elseif func.var == "Localizar" then localizarLoop()
            elseif func.var == "AutoColetar" then autoColetarLoop()
            elseif func.var == "MiniMapa" then miniMapaLoop()
            elseif func.var == "AntiQueda" then antiQuedaLoop()
            elseif func.var == "SuperForca" then superForcaLoop()
            end
        end)
        
        yPos = yPos + 40
    end
    avancadoLista.CanvasSize = UDim2.new(0, 0, 0, yPos)
end
...
function atualizarIdioma()
    ...
    btnAvancado.Text = mostrandoAvancado and textos[idiomaAtual].fecharAvancado or textos[idiomaAtual].avancado
    ...
    rightTitle.Text = mostrandoJogadores and textos[idiomaAtual].jogadores or 
                     ...
                     (mostrandoAvancado and textos[idiomaAtual].avancadoTitulo or 
...
btnAvancado.MouseButton1Click:Connect(function()
    if mostrandoAvancado then
        mostrandoAvancado = false
        mostrandoPlayers = false
        mostrandoTimes = false
        mostrandoConfig = false
        ...
        timesLista.Visible = false
        playerLista.Visible = false
        avancadoLista.Visible = true
        configLista.Visible = false
        rightTitle.Text = textos[idiomaAtual].avancadoTitulo
        btnAvancado.Text = textos[idiomaAtual].fecharAvancado
        btnTimes.Text = textos[idiomaAtual].times
        btnPlayer.Text = textos[idiomaAtual].player
        btnConfig.Text = textos[idiomaAtual].config
    end
